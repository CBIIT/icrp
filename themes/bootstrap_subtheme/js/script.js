...
*/
        ...
        $("#last-meeting-report-img")
          .attr('src', thumbnail)
          .attr('alt', meetingReport.title || 'Latest meeting report');
  ...
  $.getNewsletter = function() {
    ...
    $.get('/api/latest/newsletter').then(function(newsletter) {
      ...
      var newsletterTitle = newsletter.title || 'Latest newsletter';
      var $newsletterImage = $('<img>', {
        'class': 'center-block',
        src: thumbnail,
        alt: newsletterTitle
      });
      ...
      }).append($newsletterImage);
      $('#last_newsletter').append(
        $('<div class="row text-center"><div class="newsletter-image"></div></div>')
          .find('.newsletter-image').append($newsletterLink).end()
      );
