...
if [ "$mysql_database" == "" ]; then
	echo
	echo "SYNTAX:"
	echo "export_webform_submissions.sh [msyql database] [mysql username] [mysql password] [host]"
...
mysql $mysql_database -u$user -p$password -h$host -e 'show tables like "webform_%"' |grep -v Tables_in |grep -v mysql | xargs mysqldump $mysql_database -u$user -p$password -h$host > $target_file
...
mysql $mysql_database -u$user -p$password -h$host -e 'show tables like "webform_%"' 
